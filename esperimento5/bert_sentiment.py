...
# Carica il file csv
df = pd.read_csv("esperimento6/df_completo.csv")
...
def get_dominant_sentiment(row):
    ...
    if isinstance(row['text'], str):  # Verifica se la cella è una stringa
        probabilities = predict_sentiment(row['text'])
...
df.to_csv("esperimento6/df_sentiment.csv", index=False)
