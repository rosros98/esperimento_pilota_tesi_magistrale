...
# Funzione che applica modello senza limiti
# def predict_sentiment(sentence):
...
def get_dominant_sentiment(row):
    ...
    if isinstance(row['conversazione'], str):  # Verifica se la cella è una stringa
        probabilities = predict_sentiment(row['conversazione'])
...
df.to_csv("bert-sentiment-512.csv", index=False)
